...
def load_audio_conditioning(
    ...
    """
    Loads an audio from a path and preprocesses it for conditioning.
    Returns a Mel spectrogram of shape (batch, channels, time, mel_bins).
    """
    audio = decode_audio_from_file(path=audio_path, device=device)
    if audio is None:
        raise ValueError(f"Failed to load audio from {audio_path}")

    audio_processor = audio_processor.to(device)
    spectrogram = audio_processor.waveform_to_mel(audio)

    return spectrogram.to(dtype=dtype)
...
def decode_audio_from_file(
    ...
    """Decodes audio from a file, optionally extracting a time range.

    Always decodes sequentially from the beginning to avoid MP3 seek
    inaccuracies (decoder priming artifacts after seek).
    """
    container = av.open(path)
    try:
        try:
        ...
        for frame in container.decode(audio=audio_stream.index):
            ...
            total_decoded_samples += frame.samples
            # Stop early if we have enough data
            if end_time is not None and total_decoded_samples / sample_rate > end_time + 0.1:
                break
