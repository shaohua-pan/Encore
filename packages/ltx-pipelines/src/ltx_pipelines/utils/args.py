...
class AudioAction(argparse.Action):
    def __call__(
        self,
        parser: argparse.ArgumentParser,  # noqa: ARG002
        namespace: argparse.Namespace,
...
def basic_arg_parser(
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Audio conditioning input: path to audio file, target frame index, "
            "and conditioning strength (all three required). Default: empty list [] (no audio conditioning). "
            "Can be specified multiple times. Example: --audio path/to/audio1.wav 0 0.8 "
            "--audio path/to/audio2.wav 160 0.9"
        ),
    )
    parser.add_argument(
        "--lora",
...
def default_1_stage_arg_parser(params: PipelineParams = LTX_2_3_PARAMS) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Extra time offset (seconds) to push reference image/audio positions further into "
            "negative time, creating a temporal gap between reference and generated content. "
            "Prevents the model from treating reference as temporally adjacent to the first frame. "
            "E.g. 1.0 means ref ends at t=-1.0s instead of t=0 (default: 0.0)."
        ),
    )
    parser.add_argument(
        "--routing-scale",
        ...
        default=1.0,
        help=(
            "Extra scale applied to routing_table logits after loading them from LoRA checkpoints. "
            "The effective routing strength is `lora_strength * routing_scale`: 1.0 keeps the current behavior, "
            "0.0 makes routing nearly identity, and values between 0 and 1 damp routing without retraining."
        ),
    )
    return parser
