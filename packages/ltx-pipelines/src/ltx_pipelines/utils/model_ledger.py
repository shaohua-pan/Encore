...
class ModelLedger:
    ...
    def transformer(self) -> X0Model:
        ...
        # Load routing_table from LoRA checkpoint if present
        self._load_routing_table(model.velocity_model)
        return model

    def _load_routing_table(self, velocity_model: torch.nn.Module) -> None:
        """Detect and load routing_table from LoRA checkpoints onto the model."""
        for lora in self.loras:
            routing_key = None
            with safetensors.safe_open(lora.path, framework="pt") as f:
                keys = f.keys()
                if "diffusion_model.routing_table" in keys:
                    routing_key = "diffusion_model.routing_table"
                elif "routing_table" in keys:
                    routing_key = "routing_table"

                if routing_key is not None:
                    routing_data = f.get_tensor(routing_key).to(
                        device=velocity_model.transformer_blocks[0].attn1.to_q.weight.device,
                        dtype=torch.float32,
                    )
                    effective_scale = lora.strength * self.routing_scale
                    if effective_scale != 1.0:
                        routing_data = routing_data * effective_scale

                    velocity_model.enable_condition_routing = True
                    velocity_model.routing_table = torch.nn.Parameter(routing_data)
                    velocity_model._routing_metadata = None
                    logger.info(
                        "Loaded routing_table from %s with LoRA strength %.4f, routing scale %.4f, "
                        "effective logit scale %.4f",
                        lora.path,
                        lora.strength,
                        self.routing_scale,
                        effective_scale,
                    )
                    return  # only load from the first LoRA that has it
