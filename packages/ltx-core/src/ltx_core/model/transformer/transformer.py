...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def forward(  # noqa: PLR0915
        ...
        semantic_scale: torch.Tensor | None = None,
        sync_scale: torch.Tensor | None = None,
        ...
        if run_vx:
            ...
            v_cross_attn_out = self._apply_text_cross_attention(
                vx,
                video.context,
                self.attn2,
            ...
            if semantic_scale is not None:
                v_cross_attn_out = _apply_per_head_scale(v_cross_attn_out, semantic_scale, self.attn2.heads)
            vx = vx + v_cross_attn_out
            del v_cross_attn_out
        ...
        if run_ax:
            ...
            a_cross_attn_out = self._apply_text_cross_attention(
                ax,
                audio.context,
                self.audio_attn2,
            ...
            if semantic_scale is not None and semantic_scale.shape[0] == self.audio_attn2.heads:
                a_cross_attn_out = _apply_per_head_scale(a_cross_attn_out, semantic_scale, self.audio_attn2.heads)
            ax = ax + a_cross_attn_out
            del a_cross_attn_out
        ...
        if run_a2v or run_v2a:
            ...
            if run_a2v and not perturbations.all_in_batch(PerturbationType.SKIP_A2V_CROSS_ATTN, self.idx):
                ...
                if sync_scale is not None and sync_scale.shape[0] == self.audio_to_video_attn.heads:
                    a2v_out = _apply_per_head_scale(a2v_out, sync_scale, self.audio_to_video_attn.heads)
                vx = vx + a2v_out
                del gate_out_a2v, a2v_mask, vx_scaled, ax_scaled, a2v_out
            ...
            if run_v2a and not perturbations.all_in_batch(PerturbationType.SKIP_V2A_CROSS_ATTN, self.idx):
                ...
                if sync_scale is not None and sync_scale.shape[0] == self.video_to_audio_attn.heads:
                    v2a_out = _apply_per_head_scale(v2a_out, sync_scale, self.video_to_audio_attn.heads)
                ax = ax + v2a_out
                del gate_out_v2a, v2a_mask, ax_scaled, vx_scaled, v2a_out
...
def _apply_per_head_scale(x: torch.Tensor, scale: torch.Tensor, num_heads: int) -> torch.Tensor:
    """Apply per-head scaling to an attention output tensor.

    Args:
        x: Attention output of shape [B, T, H*d_head].
        scale: Per-head scale factors of shape [H], values in (0, 1].
        num_heads: Number of attention heads H.

    Returns:
        Scaled tensor of the same shape [B, T, H*d_head].
    """
    b, t, d = x.shape
    d_head = d // num_heads
    scale = scale.to(device=x.device, dtype=x.dtype)
    return (x.view(b, t, num_heads, d_head) * scale.view(1, 1, num_heads, 1)).view(b, t, d)