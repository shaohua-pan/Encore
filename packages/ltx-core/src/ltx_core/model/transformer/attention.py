...
class PytorchAttention(AttentionCallable):
    """PyTorch SDPA-based attention with automatic chunking for large masked attention.

    When a dense attention mask is provided, PyTorch SDPA falls back to the math
    (naive) kernel which materializes the full ``(B, H, T, T)`` attention matrix.
    For long sequences with per-head masks (e.g. routing), this causes OOM.

    This class automatically chunks the query along the sequence dimension when
    a mask is present and the sequence is long enough, processing ``chunk_size``
    query rows at a time against the full key/value set.  Each chunk only
    materializes a ``(B, H, chunk_size, T)`` slice of the attention scores.

    The mask can be in any broadcastable shape — ``(B, H, T, T)`` (full),
    ``(B, H, 1, T)`` (row-uniform routing bias), or ``(B, 1, 1, T)``
    (per-token text mask).  Broadcast masks are handled efficiently without
    materialization.
    """

    # Chunk size in query tokens.  Larger chunks reduce kernel launch overhead
    # and HBM reads of K/V at the cost of higher peak memory per chunk.
    # With T~8000, H=32: chunk=1024 → 8 chunks, ~1.5GB peak per chunk.
    CHUNK_SIZE: int = 16384 * 128  # Effectively disable chunking — 70GB free VRAM can hold full score matrix
    ...
    def __call__(
        ...
        # Only chunk self-attention (T_q == T_kv, both large) where the score
        # matrix (B, H, T, T) would be huge.  Cross-attention (T_kv << T_q,
        # e.g. text context with ~256 tokens) produces a small score matrix
        # that SDPA handles efficiently without chunking.
        use_chunked = mask is not None and seq_len > self.CHUNK_SIZE and kv_seq_len > self.CHUNK_SIZE
        ...
        if not use_chunked:
            ...
            # Chunked attention: process query in slices to avoid materializing
            # the full (B, H, T, T) attention scores matrix at once.
            # Uses torch.matmul which naturally handles broadcast dimensions.
            scale = dim_head ** -0.5
            mask_has_query_dim = mask.shape[-2] > 1
            chunks = []
            for start in range(0, seq_len, self.CHUNK_SIZE):
                end = min(start + self.CHUNK_SIZE, seq_len)
                q_chunk = q[:, :, start:end, :]  # (B, H, chunk, D)

                # Compute attention scores: (B, H, chunk, D) @ (B, H, D, T) → (B, H, chunk, T)
                attn_scores = torch.matmul(q_chunk, k.transpose(-1, -2))
                attn_scores.mul_(scale)

                # Add mask bias — handles any broadcast shape via in-place add.
                # For (B, H, 1, T) masks: broadcasts across chunk dim, no extra alloc.
                # For (B, H, T, T) masks: slice the query dim to get (B, H, chunk, T).
                if mask_has_query_dim:
                    attn_scores.add_(mask[:, :, start:end, :])
                else:
                    attn_scores.add_(mask)

                attn_weights = torch.softmax(attn_scores, dim=-1, dtype=torch.float32).to(v.dtype)
                del attn_scores
                chunk_out = torch.matmul(attn_weights, v)  # (B, H, chunk, D)
                del attn_weights
...
class AttentionFunction(Enum):
    ...
    def __call__(
        ...
        if self is AttentionFunction.PYTORCH:
            ...
            # Default: use XFormers when available and no mask, otherwise PyTorch.
            # xformers backends (FA2/FA3) on newer GPUs do not support raw tensor
            # attn_bias, so fall back to the chunked PyTorch path when a mask is given.
            if memory_efficient_attention is not None and mask is None:
                return XFormersAttention()(q, k, v, heads, mask)
            else:
                return PytorchAttention()(q, k, v, heads, mask)
