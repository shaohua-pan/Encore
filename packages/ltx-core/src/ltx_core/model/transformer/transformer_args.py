...
class TransformerArgsPreprocessor:
    ...
    def _prepare_self_attention_mask(
        ...
        Input shape: ``(B, T, T)`` or ``(B, 1, T)`` with values in [0, 1].
        Output shape: ``(B, 1, T, T)`` or ``(B, 1, 1, T)`` (head-broadcast dim inserted).

        A compact ``(B, 1, T)`` input produces ``(B, 1, 1, T)`` — a row-uniform
        mask that broadcasts across all query positions.  This avoids allocating
        the full T×T matrix when the mask is the same for every query.
        ...
        return bias.unsqueeze(1)  # (B, 1, ...) for head broadcast
