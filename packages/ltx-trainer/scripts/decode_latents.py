...
class LatentsDecoder:
    ...
    def _process_file(self, latent_file: Path, output_dir: Path, seed: int | None) -> None:
        ...
        if "ref_image_latent" in data:
            ref_image_latent = data["ref_image_latent"]  # [C, 1, H', W']
            
            # Add batch dimension: [C, 1, H, W] -> [1, C, 1, H, W]
            ref_image_latent = ref_image_latent.unsqueeze(0).to(device=self.device, dtype=torch.bfloat16)

            # Decode the reference image
            reference_image = self.vae(ref_image_latent)  # [B, C, 1, H, W]

            # Convert to [1, C, H, W] format and normalize to [0, 1]
            reference_image = reference_image[0]  # Remove batch dimension -> [C, 1, H, W]
            reference_image = reference_image.permute(1, 0, 2, 3)  # [C, 1, H, W] -> [1, C, H, W]
            reference_image = (reference_image + 1) / 2  # Denormalize from [-1, 1] to [0, 1]
            reference_image = reference_image.clamp(0, 1)
    ...
    def _process_audio_file(self, latent_file: Path, output_dir: Path) -> None:
        ...
        audio_numpy = waveform[0].detach().float().cpu().numpy().T
        sf.write(str(output_path), audio_numpy, sample_rate, format='WAV', subtype='PCM_16')

        # Decode reference audio if present (analogous to ref_image_latent in _process_file)
        if "ref_audio_latent" in data:
            ref_audio_latent = data["ref_audio_latent"].to(device=self.device, dtype=torch.bfloat16)

            # Add batch dimension: [C, T, F] -> [1, C, T, F]
            ref_audio_latent = ref_audio_latent.unsqueeze(0)

            # Decode reference audio latent -> mel spectrogram -> waveform
            ref_mel = self.audio_vae(ref_audio_latent)
            ref_waveform = self.vocoder(ref_mel)

            # Save as WAV with _ref suffix
            ref_output_path = output_dir / f"{latent_file.stem}_ref.wav"
            ref_audio_numpy = ref_waveform[0].detach().float().cpu().numpy().T
            sf.write(str(ref_output_path), ref_audio_numpy, sample_rate, format='WAV', subtype='PCM_16')
