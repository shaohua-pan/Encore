...
class MediaDataset(Dataset):
    ...
    def __init__(
        ...
        audio_indices_column: str | None = None,
    ) -> None:
        """
        ...
        Args:
            ...
            with_reference_image: Whether to extract reference image
            with_reference_audio: Whether to extract reference audio (2.5s random crop from generated audio)
            video_prefix_path: Prefix path to remove from video paths
            audio_indices_column: Column name for audio indices in the metadata file
        Raises:
            ValueError: If the specified reshape mode is not supported
        """
        super().__init__()
        ...
        self.with_reference_image = with_reference_image
        self.with_reference_audio = with_reference_audio
        self.video_prefix_path = video_prefix_path
        self.audio_indices_column = audio_indices_column
        ...
        # Load audio indices
        self.audio_indices = self._load_audio_indices(audio_indices_column) if audio_indices_column else None
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        audio_indices = self.audio_indices[index] if self.audio_indices else None

        # Compute relative path of the video
        data_root = self.dataset_file.parent
        
        if self.video_prefix_path is not None:
            # compute relative path when video_path is not in data root
            relative_path = str(video_path.relative_to(self.video_prefix_path))
            media_relative_path = str(self.main_media_paths[index].relative_to(self.video_prefix_path))
        else:
            # compute relative path when video_path is in data root
        ...
        if video_path.suffix.lower() in [".png", ".jpg", ".jpeg"]:
            ...
            media_tensor, fps, ref_image = self._preprocess_video(video_path, self.with_reference_image, audio_indices)
            ...
            if self.with_audio:
                ...
                audio_start = audio_indices[0] / fps if audio_indices is not None else 0
                audio_data, full_waveform, sample_end = self._extract_audio(video_path, target_duration, audio_start)
            else:
                audio_data = None
                full_waveform = None
                sample_end = 0
        ...
        if audio_data is not None:
            ...
            # Extract reference audio if enabled (random 2.5s crop from after main audio)
            if self.with_reference_audio and full_waveform is not None:
                ref_audio_data = self._extract_reference_audio(
                    full_waveform, audio_data["sample_rate"], sample_end
                )
                if ref_audio_data is not None:
                    result["reference_audio"] = ref_audio_data
    ...
    @staticmethod
    def _extract_audio(
        video_path: Path, target_duration: float, audio_start: int
    ) -> tuple[dict[str, torch.Tensor | int] | None, torch.Tensor | None, int]:
        """Extract audio track from a video file, trimmed to match video duration.

        Returns:
            Tuple of (audio_data, full_waveform, sample_end):
            - audio_data: Dict with trimmed waveform and sample_rate, or None on failure
            - full_waveform: The original untrimmed waveform for reference audio extraction
            - sample_end: The end sample index of the main audio in the full waveform
        """
        try:
            ...
            waveform_np, sample_rate = librosa.load(video_path, sr=None, mono=False)
            waveform = torch.from_numpy(waveform_np)
            if waveform.dim() == 1:
                waveform = waveform.unsqueeze(0)
            waveform = waveform.float()
            # Ensure stereo (2 channels) - audio VAE encoder expects 2-channel input
            ...
            if current_samples > target_samples:
                ...
                sample_start = int(audio_start * sample_rate)
                sample_end = sample_start + target_samples
                waveform = waveform[..., sample_start:sample_end]
            elif current_samples < target_samples:
                # Pad with zeros to target duration
                padding = target_samples - current_samples
                waveform = torch.nn.functional.pad(waveform, (0, padding))
                sample_end = current_samples
                logger.warning(f"Padded audio to {target_duration:.2f} seconds for {video_path}")
            else:
                sample_end = current_samples

            return {"waveform": waveform, "sample_rate": sample_rate}, full_waveform, sample_end
    ...
    def _extract_reference_audio(
        ...
        sample_rate: int,
        sample_end: int,
        ref_duration: float = 2.5,
    ) -> dict[str, torch.Tensor | int] | None:
        """Extract reference audio from after the main audio segment to avoid overlap.

        Randomly selects a segment of `ref_duration` seconds from the portion of
        the full waveform that comes AFTER `sample_end`. If the remaining audio
        is shorter than `ref_duration`, pads with silence (zeros).

        Args:
            full_waveform: The original untrimmed waveform [channels, samples]
            sample_rate: Audio sample rate
            sample_end: End sample index of the main audio in the full waveform
            ref_duration: Target duration in seconds for the reference audio (default: 2.5s)

        Returns:
            Dict with {"waveform": Tensor[channels, ref_samples], "sample_rate": int},
            or None if no audio remains after sample_end.
        """
        ref_samples = int(ref_duration * sample_rate)
        total_samples = full_waveform.shape[-1]
        remaining_samples = total_samples - sample_end

        if remaining_samples <= 0:
            # No audio after sample_end, return silence
            ref_waveform = torch.zeros(*full_waveform.shape[:-1], ref_samples)
            return {"waveform": ref_waveform, "sample_rate": sample_rate}

        remaining_waveform = full_waveform[..., sample_end:]

        if remaining_samples >= ref_samples:
            # Randomly select a start position within the remaining audio
            start = random.randint(0, remaining_samples - ref_samples)
            ref_waveform = remaining_waveform[..., start:start + ref_samples]
        else:
            # Remaining audio shorter than ref_duration, pad with silence
            padding = ref_samples - remaining_samples
            ref_waveform = torch.nn.functional.pad(remaining_waveform, (0, padding))
    ...
    def _load_audio_indices(self, column: str) -> list[list[int]]:
        """Load audio index from the specified data source."""
        if self.dataset_file.suffix == ".csv":
            return self._load_audio_indices_from_csv(column)
        elif self.dataset_file.suffix == ".json":
            return self._load_audio_indices_from_json(column)
        elif self.dataset_file.suffix == ".jsonl":
            return self._load_audio_indices_from_jsonl(column)
        else:
            raise ValueError("Expected `dataset_file` to be a path to a CSV, JSON, or JSONL file.")

    def _load_audio_indices_from_csv(self, column: str) -> list[list[int]]:
        """Load audio index from a CSV file."""
        df = pd.read_csv(self.dataset_file)
        if column not in df.columns:
            raise ValueError(f"Column '{column}' not found in CSV file")

        audio_index = [json.loads(line) for line in df[column].tolist()]
        return audio_index

    def _load_audio_indices_from_json(self, column: str) -> list[list[int]]:
        """Load audio index from a JSON file."""
        with open(self.dataset_file, "r", encoding="utf-8") as file:
            data = json.load(file)
        ...
        for entry in data:
            if column not in entry:
                raise ValueError(f"Key '{column}' not found in JSON entry")
            audio_index.append(entry[column])

        return audio_index

    def _load_audio_indices_from_jsonl(self, column: str) -> list[list[int]]:
        """Load audio index from a JSONL file."""
        audio_index = []
        with open(self.dataset_file, "r", encoding="utf-8") as file:
            for line in file:
                entry = json.loads(line)
    ...
    def _filter_valid_videos(self) -> None:
        ...
        valid_audio_indices = [] if self.audio_indices_column else None
        ...
        for i, video_path in enumerate(self.video_paths):
            if video_path.suffix.lower() in [".png", ".jpg", ".jpeg"]:
                ...
                if self.audio_indices_column:
                    valid_audio_indices.append(self.audio_indices[i])
                continue
        ...
        self.audio_indices = valid_audio_indices
    ...
    def _preprocess_video(self, path: Path, with_reference_image: bool = False, audio_indices: list[int] | None = None) -> tuple[torch.Tensor, float]:
        """Preprocess a video by loading, resizing, and applying transforms.
        Returns:
            Tuple of (video tensor in [C, F, H, W] format, fps)
        """
        # Load video frames up to max_target_frames or use the audio indices to select frames
        video, fps = read_video(path, max_frames=self.max_target_frames if not audio_indices else None)

        nearest_bucket = self._get_resolution_bucket_for_item(video)
        target_num_frames, target_height, target_width = nearest_bucket
        if audio_indices is not None and len(audio_indices) != target_num_frames:
            raise ValueError(f"Expected audio indices for {target_num_frames} frames, but got {len(audio_indices)}")
        frames_resized = self._resize_and_crop(video, target_height, target_width)

        ref_img = None
        if with_reference_image:
            # Use the frame outside the trimmed video as the reference image
            if len(frames_resized) <= target_num_frames:
                ref_img = frames_resized[-1].clone()
            else:
                ref_img = frames_resized[random.randint(target_num_frames, len(frames_resized) - 1)].clone()
            ref_img = self.transforms(ref_img)
            ref_img = ref_img.unsqueeze(0).permute(1, 0, 2, 3).contiguous()

        # Trim video to target number of frames
        if audio_indices is not None:
            frames_resized = frames_resized[audio_indices]
        else:
...
def compute_latents(  # noqa: PLR0913, PLR0915
    ...
    Args:
        ...
        with_reference_image: Whether to include a reference image
        with_reference_audio: Whether to extract and encode reference audio (2.5s random crop)
        video_prefix_path: Prefix path containing all video paths in the dataset
        audio_indices_column: Column name for audio indices in the metadata file
    """
    ...
    dataset = MediaDataset(
        ...
        with_reference_image=with_reference_image,
        with_reference_audio=with_reference_audio,
        video_prefix_path=video_prefix_path,
        audio_indices_column=audio_indices_column
    )
    ...
    with Progress(
        ...
        for batch_idx, batch in enumerate(dataloader):
            # Log progress every 10 batches for monitoring in log files
            if (batch_idx + 1) % 10 == 0:
            ...
            with torch.inference_mode():
                ...
                if with_reference_image:
                    ...
                    video_latent_data["ref_image_latent"] = encode_video(vae=vae, video=reference_image)["latents"]
            ...
            for i in range(len(batch["relative_path"])):
                ...
                if with_reference_image:
                    latent_data["ref_image_latent"] = video_latent_data["ref_image_latent"][i].cpu().contiguous()  # [C, 1, H', W']
                ...
                if with_audio:
                    ...
                    if audio_batch is not None:
                        ...
                        # Encode and save reference audio if enabled
                        if with_reference_audio:
                            ref_audio_batch = batch.get("reference_audio")
                            if ref_audio_batch is not None:
                                ref_audio_data = Audio(
                                    waveform=ref_audio_batch["waveform"][i],
                                    sampling_rate=ref_audio_batch["sample_rate"][i].item(),
                                )
                                with torch.inference_mode():
                                    ref_audio_latents = encode_audio(audio_vae_encoder, audio_processor, ref_audio_data)
                                audio_save_data["ref_audio_latent"] = ref_audio_latents["latents"].cpu().contiguous()  # [C, T, F]
                                audio_save_data["ref_audio_num_time_steps"] = ref_audio_latents["num_time_steps"]
                                audio_save_data["ref_audio_duration"] = ref_audio_latents["duration"]
...
def main(  # noqa: PLR0913
    ...
    with_reference_image: bool = typer.Option(
        default=False,
        help="Extract and encode reference image",
    ),
    with_reference_audio: bool = typer.Option(
        default=False,
        help="Extract and encode reference audio (2.5s random crop from generated audio)",
    ),
    video_prefix_path: str | None = typer.Option(
        default=None,
        help="Prefix path containing video paths when saving latents (e.g. '/root/data/')",
    ),
    audio_indices_column: str = typer.Option(
        default=None,
        help="Column name in the dataset JSON/JSONL/CSV file containing audio indices",
    ),
) -> None:
