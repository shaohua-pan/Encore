...
from ltx_trainer.training_strategies.text_to_video_encore import TextToVideoEncoreConfig
...
TrainingStrategyConfig = Annotated[
    ...
    | Annotated[TextToVideoEncoreConfig, Tag("text_to_video_encore")],
...
class OptimizationConfig(ConfigBaseModel):
    ...
    routing_table_lr_multiplier: float = Field(
        default=1.0,
        description=(
            "Multiplier applied to the base learning_rate for the Condition "
            "Routing Table (routing_table) parameter group. Use >1 (e.g. 100) "
            "to let the 6k-param routing tensor move faster than the LoRA "
            "adapters. Has no effect when condition routing is disabled."
        ),
    )
...
class DataConfig(ConfigBaseModel):
    ...
    skip_integrity_check: bool = Field(
        default=False,
        description="Skip checking if corresponding files exist in all data sources during dataset loading. "
        "This significantly speeds up dataset initialization when you can guarantee all samples are complete.",
    )

    saved_data_files_path: str | None = Field(
        default=None,
        description="Path to a file containing a list of sample files to use for training. "
    )
...
class ValidationConfig(ConfigBaseModel):
    ...
    audios: list[str] | None = Field(
        default=None,
        description="List of audio paths to use for validation (for reference audio conditioning). "
        "One audio path must be provided for each validation prompt",
    )
