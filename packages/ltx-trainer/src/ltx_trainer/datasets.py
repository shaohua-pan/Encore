...
class _PathCompatUnpickler(pickle.Unpickler):
    """Handle pickle files created with Python 3.12+ where PosixPath/WindowsPath
    moved to pathlib._local, which doesn't exist in Python 3.10/3.11."""
...
class PrecomputedDataset(Dataset):
    def __init__(self,
                 ...
                 saved_data_files_path: str | None = None,
    ) -> None:
        """
        ...
        Args:
            ...
            skip_integrity_check: If True, skip checking if corresponding files exist
              in all data sources. This significantly speeds up dataset loading when
              you can guarantee all samples are complete. Default is False.
            saved_data_files_path: Path to a file containing a list of sample files to use.
        Example:
            # Standard mode (list)
        ...
        self.saved_data_files_path = saved_data_files_path
        self._use_absolute_paths = False
    ...
    def _setup_source_paths(self) -> dict[str, Path]:
        ...
        for dir_name in self.data_sources:
            ...
            # Skip check when saved_data_files_path is provided, as data may be
            # spread across multiple subdirectories (e.g., subset_a/, subset_b/).
            if not source_path.exists() and self.saved_data_files_path is None:
                raise FileNotFoundError(f"Required {dir_name} directory does not exist: {source_path}")

        return source_paths

    def _discover_samples(self, saved_data_files_path: str | None = None) -> dict[str, list[Path]]:
        ...
        if self.saved_data_files_path is not None:
            with open(self.saved_data_files_path, "rb") as f:
                data_files = _PathCompatUnpickler(f).load()
            logger.info(f"Loaded data_files from {self.saved_data_files_path}.")

            # Check if files are absolute paths that can't be relativized to source_path.
            # This handles multi-subdirectory structures like:
            #   preprocessed_data_root/subset_a/latents/xxx.pt
            #   preprocessed_data_root/subset_b/latents/xxx.pt
            if data_files:
                first_file = Path(data_files[0])
                needs_absolute = False
                if first_file.is_absolute():
                    try:
                        first_file.relative_to(data_path)
                    except ValueError:
                        needs_absolute = True
                if needs_absolute:
                    self._use_absolute_paths = True
                    return self._discover_samples_absolute(data_files, data_key)
        else:
        ...
        if self.skip_integrity_check:
            # Fast path: assume all samples are complete, skip file existence checks
            logger.info(
                f"Skipping integrity check for {len(data_files)} samples "
                "(skip_integrity_check=True). Assuming all data sources have matching files."
            )
            for data_file in data_files:
                rel_path = data_file.relative_to(data_path)
                self._fill_sample_data_files(data_file, rel_path, sample_files)
        else:
            # Standard path: verify each sample has matching files in all sources
            for data_file in data_files:
                rel_path = data_file.relative_to(data_path)
    ...
    def _discover_samples_absolute(self, data_files: list, ref_data_key: str) -> dict[str, list[Path]]:
        """Discover samples when saved_data_files_path contains absolute paths
        pointing to files in multiple subdirectories.

        For each absolute latent path like:
            .../subset_a/latents/xxx.pt
        derives other source paths by replacing the directory component:
            .../subset_a/conditions/xxx.pt
            .../subset_a/audio_latents/xxx.pt
        """
        sample_files = {output_key: [] for output_key in self.data_sources.values()}
        ...
        for data_file in data_files:
            ...
            for dir_name, output_key in self.data_sources.items():
                if dir_name == ref_data_key:
                    sample_files[output_key].append(data_file)
                else:
                    # Replace the reference source dir name in the path with target dir name
                    file_str = str(data_file)
                    target_file = Path(file_str.replace(f"/{ref_data_key}/", f"/{dir_name}/", 1))

                    # Handle legacy naming: latent_X.pt -> condition_X.pt
                    if dir_name == "conditions" and data_file.name.startswith("latent_"):
                        target_file = target_file.parent / f"condition_{data_file.stem[7:]}.pt"

                    sample_files[output_key].append(target_file)

        if not any(sample_files.values()):
            raise ValueError(f"No data files found from {self.saved_data_files_path}")

        logger.info(
            f"Loaded {len(data_files)} samples with absolute paths "
            f"from {self.saved_data_files_path} (multi-subdirectory mode)."
        )
        return sample_files
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        for dir_name, output_key in self.data_sources.items():
            if self._use_absolute_paths:
                file_path = self.sample_files[output_key][index]
            else:
                source_path = self.source_paths[dir_name]
