...
# Use global RANK (not LOCAL_RANK) so only one process across all nodes is main
...
class LtxvTrainer:
        ...
        with progress:
            ...
            for step in range(cfg.optimization.steps * cfg.optimization.gradient_accumulation_steps):
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    total_loss = self._training_step(batch)

                    if type(total_loss) is tuple:
                        loss = total_loss[0]
                        video_loss = total_loss[1]
                        audio_loss = total_loss[2]
                    else:
                        loss = total_loss
                        video_loss, audio_loss = 0, 0
    ...
    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> Tensor:
        ...
        # Set condition routing metadata on the base LTXModel (if routing is enabled).
        # Must navigate through Accelerate/PeftModel wrappers because __setattr__
        # on wrapper objects won't propagate to the underlying LTXModel instance.
        base_model = self._accelerator.unwrap_model(self._transformer)
        if hasattr(base_model, 'get_base_model'):
            base_model = base_model.get_base_model()
        if hasattr(base_model, 'enable_condition_routing') and base_model.enable_condition_routing:
            base_model._routing_metadata = model_inputs.routing_metadata
        ...
        # Clear routing metadata after forward pass
        if hasattr(base_model, '_routing_metadata'):
            base_model._routing_metadata = None
    ...
    def _load_models(self) -> None:
        ...
        # Check if we need Audio VAE encoder (for reference audio conditioning)
        need_audio_vae_encoder = self._config.validation.audios is not None
        ...
        components = load_ltx_model(
            ...
            with_audio_vae_encoder=need_audio_vae_encoder, # Needed for audio conditioning
        ...
        # Enable condition routing if the training strategy requests it
        if hasattr(self._training_strategy.config, 'enable_condition_routing') and self._training_strategy.config.enable_condition_routing:
            self._transformer.enable_condition_routing = True
            self._transformer.routing_table = torch.nn.Parameter(
                torch.zeros((len(self._transformer.transformer_blocks), self._transformer.num_attention_heads, 4))
            )
            self._transformer._routing_metadata = None
            logger.info(
                f"Condition routing enabled: routing_table shape = {self._transformer.routing_table.shape} "
                f"({self._transformer.routing_table.numel()} params)"
            )
        ...
        if self._audio_encoder is not None:
            self._audio_encoder = self._audio_encoder.to(dtype=torch.bfloat16)
    ...
    def _collect_trainable_params(self) -> None:
        ...
        # Unfreeze routing_table if condition routing is enabled
        # (routing_table is always trainable regardless of LoRA/full mode)
        base_model = self._transformer.get_base_model() if hasattr(self._transformer, "get_base_model") else self._transformer
        if hasattr(base_model, 'routing_table') and base_model.routing_table is not None:
            base_model.routing_table.requires_grad_(True)
    ...
    def _load_lora_checkpoint(self, checkpoint_path: Path) -> None:
        ...
        # Extract and load routing_table separately (not managed by PEFT)
        # get_base_model() returns LTXModel (unwraps PeftModel → LoraModel → model)
        base_model = self._transformer.get_base_model()
        if "routing_table" in state_dict:
            routing_table_data = state_dict.pop("routing_table")
            if hasattr(base_model, 'routing_table') and base_model.routing_table is not None:
                target_shape = base_model.routing_table.shape
                if routing_table_data.shape != target_shape:
                    if routing_table_data.numel() == base_model.routing_table.numel():
                        routing_table_data = routing_table_data.reshape(target_shape)
                    else:
                        raise ValueError(
                            f"routing_table shape mismatch: ckpt={tuple(routing_table_data.shape)} "
                            f"vs model={tuple(target_shape)} (numel differs)."
                        )
                base_model.routing_table.data.copy_(routing_table_data)
                logger.info("✅ Routing table loaded from checkpoint")
    ...
    def _prepare_models_for_training(self) -> None:
        ...
        if self._audio_encoder is not None:
            self._audio_encoder = self._audio_encoder.to("cpu")
    ...
    def _init_dataloader(self) -> None:
        ...
        if self._dataset is None:
            ...
            self._dataset = PrecomputedDataset(
                ...
                skip_integrity_check=self._config.data.skip_integrity_check,
                saved_data_files_path=self._config.data.saved_data_files_path
            )
    ...
    def _build_param_groups(self, base_lr: float) -> list[dict]:
        """Split trainable params into (routing_table, rest) groups when a
        non-unit ``routing_table_lr_multiplier`` is configured. The routing
        table is identified by ``id(...)`` against the unwrapped transformer."""
        multiplier = self._config.optimization.routing_table_lr_multiplier
        base_model = (
            self._transformer.get_base_model()
            if hasattr(self._transformer, "get_base_model")
            else self._transformer
        )
        routing_param = getattr(base_model, "routing_table", None)
        if routing_param is None or multiplier == 1.0:
            return [{"params": self._trainable_params, "lr": base_lr}]

        routing_id = id(routing_param)
        routing_group: list[torch.nn.Parameter] = []
        other_group: list[torch.nn.Parameter] = []
        for p in self._trainable_params:
            (routing_group if id(p) == routing_id else other_group).append(p)

        routing_lr = base_lr * multiplier
        logger.info(
            "Optimizer param groups: %d 'other' params @ lr=%.3e, "
            "%d routing_table params @ lr=%.3e (multiplier=%.2f)",
            sum(p.numel() for p in other_group),
            base_lr,
            sum(p.numel() for p in routing_group),
            routing_lr,
            multiplier,
        )
        groups: list[dict] = []
        if other_group:
            groups.append({"params": other_group, "lr": base_lr})
        if routing_group:
            groups.append({"params": routing_group, "lr": routing_lr})
        return groups
    ...
    def _sample_videos(self, progress: TrainingProgress) -> list[Path] | None:
        ...
        use_audios = self._config.validation.audios is not None
        ...
        sampler = ValidationSampler(
            ...
            audio_encoder=self._audio_encoder if use_audios else None,
        ...
        for prompt_idx, prompt in enumerate(self._config.validation.prompts):
            ...
            if use_audios:
                audio_path = self._config.validation.audios[prompt_idx]
                # Load audio [C, samples] using librosa (matches process_videos.py)
                # Auto-resample to audio_encoder's sample rate
                target_sr = self._audio_encoder.sample_rate if self._audio_encoder is not None else 24000
                waveform_np, _ = librosa.load(audio_path, sr=target_sr, mono=False)
                reference_audio = torch.from_numpy(waveform_np)
            ...
            gen_config = GenerationConfig(
                ...
                ref_position_offset=getattr(self._config.training_strategy, "ref_position_offset", 0.0),
            )
    ...
    def _save_checkpoint(self) -> Path | None:
        ...
        if is_lora:
            ...
            # Include routing_table if condition routing is enabled (not captured by PEFT)
            base_model = unwrapped.get_base_model() if hasattr(unwrapped, 'get_base_model') else unwrapped
            if hasattr(base_model, 'routing_table') and base_model.routing_table is not None:
                state_dict["routing_table"] = base_model.routing_table.data
