...
from ltx_core.types import AudioLatentShape, LatentState, SpatioTemporalScaleFactors, VideoLatentShape, VideoPixelShape, Audio
...
class GenerationConfig:
    ...
    reference_audio: Tensor | None = None  # Optional reference audio [C, samples]
    ...
    ref_position_offset: float = 0.0  # Extra time offset (seconds) to isolate ref from first frame positions
...
class ValidationSampler:
    ...
    def __init__(
        ...
        Args:
            ...
            audio_encoder: Optional audio VAE encoder (for reference audio conditioning)
        ...
        # Initialize AudioProcessor if audio encoder is present
        self._audio_processor = None
        if self._audio_encoder is not None:
            self._audio_processor = AudioProcessor(
                target_sample_rate=self._audio_encoder.sample_rate,
    ...
    def generate(
        ...
        if config.reference_audio is not None:
            return self._generate_with_encore(config, device)
    ...
    def _generate_with_encore(self, config: GenerationConfig, device: torch.device) -> tuple[Tensor, Tensor | None]:
        """Generate with Encore-style conditioning (reference image + reference audio).
        In Encore mode:
        - Reference image (same as condition_image) is prepended to the video latents
        - Reference audio is prepended to the audio latents
        - First frame audio corresponding to the input image replaces noise
        """
        # Get prompt embeddings (from cache or encode on-the-fly)
        v_ctx_pos, a_ctx_pos, v_ctx_neg, a_ctx_neg = self._get_prompt_embeddings(config, device)
        ...
        # --- Video Handling ---
        # Create target video state (positions are generated with its own causal structure)
        video_tools = self._create_video_latent_tools(config)
        target_clean_state = video_tools.create_initial_state(device=device, dtype=torch.bfloat16)

        # Encode reference image (same as condition_image in Encore mode)
        if config.condition_image is None:
            raise ValueError("Encore mode requires condition_image to be provided as reference image")

        ref_image_encoded = self._encode_conditioning_image(config.condition_image, config.height, config.width, device)

        # TODO: this is useful?
        # noise_scale = 0.05
        # noise = torch.randn_like(ref_image_encoded) * noise_scale 
        # ref_image_encoded = ref_image_encoded + noise

        ref_image_patchified = self._video_patchifier.patchify(ref_image_encoded)  # [1, num_patches, C]
        ref_seq_len = ref_image_patchified.shape[1]

        # Apply first-frame image conditioning to target (use the same image)
        target_clean_state = self._apply_image_conditioning(
            target_clean_state, config.condition_image, config, device
        )

        # Compute positions for the combined sequence (reference + target).
        # ref_image and video are encoded SEPARATELY, each with its own causal VAE.
        # We must generate positions independently and concatenate, matching training logic:
        #   ref_positions:  1 frame  (causal first = 1px)
        #   vid_positions:  N frames (own causal: frame0=1px, frame1+=8px)
        # Offset video by 1/fps, concatenate, then shift all by -1/fps so ref is at negative time.
        ref_shape = VideoLatentShape(
            batch=1, channels=target_clean_state.latent.shape[-1],
            frames=1, height=config.height // 32, width=config.width // 32,
        )
        ref_coords = self._video_patchifier.get_patch_grid_bounds(output_shape=ref_shape, device=device)
        ref_positions = get_pixel_coords(ref_coords, scale_factors=VIDEO_SCALE_FACTORS, causal_fix=True)
        ref_positions = ref_positions.to(torch.bfloat16)
        ref_positions[:, 0, ...] /= config.frame_rate

        # target_clean_state.positions already has correct causal positions for the video
        vid_positions = target_clean_state.positions.clone()
        # Offset video temporal axis by ref's pixel extent (1 pixel = 1/fps sec)
        vid_positions[:, 0, ...] += (1.0 / config.frame_rate)
        ...
        if config.ref_position_offset > 0:
            ref_positions[:, 0, ...] -= config.ref_position_offset
        ...
        combined_video_positions[:, 0, ...] -= (1.0 / config.frame_rate)
        ...
        combined_clean_state = LatentState(
            latent=torch.cat([ref_image_patchified.to(torch.bfloat16), target_clean_state.latent], dim=1),
            denoise_mask=torch.cat([ref_denoise_mask, target_clean_state.denoise_mask], dim=1),
            positions=combined_video_positions,
            clean_latent=torch.cat([ref_image_patchified.to(torch.bfloat16), target_clean_state.clean_latent], dim=1),
        )
        ...
        if config.generate_audio:
            ...
            audio_target_clean_state = audio_tools.create_initial_state(device=device, dtype=torch.bfloat16)

            # Audio context conditioning: the first 2 audio tokens correspond to the
            # conditioning video frame (1 pixel frame = 1/fps sec). Audio VAE is causal:
            # token0 covers ~0.01s, token1 covers ~0.04s → 2 tokens cover ~0.05s >= 1/fps.
            # This matches the training logic in _prepare_audio_inputs.
            audio_context_tokens = 2

            # Apply reference audio if provided
            if config.reference_audio is not None:
                # 1. Encode reference audio
                ref_audio_latent, ref_audio_positions, ref_audio_shape = self._encode_audio(config.reference_audio, device)
                ref_audio_seq_len = ref_audio_latent.shape[1]

                # 2. Condition the first audio_context_tokens in target (corresponding to
                # the conditioning video frame / input image), matching training logic.
                tokens_to_copy = min(ref_audio_seq_len, audio_context_tokens)
                audio_target_clean_state.latent[:, :tokens_to_copy] = ref_audio_latent[:, :tokens_to_copy]
                audio_target_clean_state.clean_latent[:, :tokens_to_copy] = ref_audio_latent[:, :tokens_to_copy]
                audio_target_clean_state.denoise_mask[:, :tokens_to_copy] = 0.0

                # 3. Create combined state (Reference Audio + Target Audio).
                # Generate positions separately and concatenate with time offset,
                # matching the audio position shift logic in training.
                ref_audio_denoise_mask = torch.zeros(1, ref_audio_seq_len, 1, device=device, dtype=torch.float32)

                # target audio positions already correct from create_initial_state
                target_audio_positions = audio_target_clean_state.positions.clone()

                # Offset target audio by ref_audio's time extent, then shift ref to negative time.
                # This matches training's audio position shift logic:
                # ref_audio_time = _get_audio_latent_time_in_sec(ref_audio_len, ref_audio_len+1)
                ref_audio_time = self._audio_patchifier._get_audio_latent_time_in_sec(
                    ref_audio_seq_len, ref_audio_seq_len + 1,
                    dtype=target_audio_positions.dtype, device=device,
                ).item()
                target_audio_positions[:, 0, :, :] += ref_audio_time
                combined_audio_positions = torch.cat([ref_audio_positions, target_audio_positions], dim=2)
                combined_audio_positions[:, 0, :, :] -= ref_audio_time
                # Additionally shift only ref audio tokens further into negative time
                if config.ref_position_offset > 0:
                    combined_audio_positions[:, 0, :ref_audio_seq_len, :] -= config.ref_position_offset

                audio_clean_state = LatentState(
                    latent=torch.cat([ref_audio_latent, audio_target_clean_state.latent], dim=1),
                    denoise_mask=torch.cat([ref_audio_denoise_mask, audio_target_clean_state.denoise_mask], dim=1),
                    positions=combined_audio_positions,
                    clean_latent=torch.cat([ref_audio_latent, audio_target_clean_state.clean_latent], dim=1),
                )
            else:
                audio_clean_state = audio_target_clean_state
        ...
        audio_output = None
        if audio_state is not None and audio_tools is not None:
            # Strip reference audio from latent if present
            if ref_audio_seq_len > 0:
                audio_state = replace(
                    audio_state,
                    latent=audio_state.latent[:, ref_audio_seq_len:],
                    denoise_mask=audio_state.denoise_mask[:, ref_audio_seq_len:],
                    positions=audio_state.positions[:, :, ref_audio_seq_len:],
                )
    ...
    def _encode_audio(self, audio: Tensor, device: torch.device) -> tuple[Tensor, Tensor, AudioLatentShape]:
        """Encode audio to patchified latents and compute positions.
        Args:
            audio: Audio tensor [C, samples]
            device: Device to run encoding on
        Returns:
            Tuple of (patchified_latents, positions, latent_shape)
        """
        # Ensure batch dimension [1, C, samples]
        if audio.ndim == 2:
        ...
        with torch.autocast(device_type=str(device).split(":")[0], dtype=torch.bfloat16):
            ...
            # Note: We assume audio is already at the correct sample rate (self._audio_encoder.sample_rate)
            mel_spectrogram = self._audio_processor.waveform_to_mel(
                Audio(waveform=audio, sampling_rate=self._audio_encoder.sample_rate)
            )

            # Encode Mel Spectrogram to latents
            # Audio encoder expects [B, C, F, T] (freq, time) or [B, C, T, F]?
            # process_videos.py: latents = audio_vae_encoder(mel_spectrogram)
            # ltx-core AudioEncoder usually expects [B, 1, F, T] for spectrogram?
            # process_videos.py says: latents shape: [batch, channels, time, freq]
            latents = self._audio_encoder(mel_spectrogram)

        self._audio_encoder.to("cpu")
        self._audio_processor.to("cpu")

        # Patchify: [B, C, T, F] -> [B, seq_len, C]
        latents = latents.to(torch.bfloat16)
        patchified = self._audio_patchifier.patchify(latents)

        # Compute positions
        # Latent shape from encoder output: [B, C, T, F]
        # But we patchified it. We need shape for get_patch_grid_bounds.
        latent_shape = AudioLatentShape(
            batch=1,
            channels=latents.shape[1],
            frames=latents.shape[2],  # T
            mel_bins=latents.shape[3],  # F
        )
        latent_coords = self._audio_patchifier.get_patch_grid_bounds(output_shape=latent_shape, device=device)
        positions = latent_coords.to(torch.bfloat16)

        return patchified, positions, latent_shape
    ...
    def _validate_config(self, config: GenerationConfig) -> None:
        ...
        if config.reference_audio is not None and self._audio_encoder is None:
            raise ValueError("Reference audio conditioning requires audio_encoder")
